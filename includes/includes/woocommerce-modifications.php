    ...
    if ($product_type === 'camp') {
        ...
        if ($total_sessions > 0 && $remaining_sessions > 0 && $remaining_sessions <= $total_sessions) {
            if ($remaining_sessions < $total_sessions && $session_rate > 0) {
                $price = $session_rate * $remaining_sessions; // Use session rate * remaining sessions
            } elseif ($remaining_sessions == $total_sessions && $session_rate > 0) {
                $price = $session_rate * $remaining_sessions; // Full price using session rate
            } else {
                // Backwards compatibility: prorate base price if no session rate
                $price = ($price / $total_sessions) * $remaining_sessions;
            }
            $price = max(0, $price);
