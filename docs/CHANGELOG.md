...
### Fixed
- Retain Base Price on Order Meta Repair: repair tool no longer strips `_base_price` from order line items, preserving accurate discount calculations on repaired orders.
- Camp Week Index hidden from customer surfaces only (thank-you pages, emails, My Account); data retained for admin order views and Reports & Rosters.

See [PR #43](https://github.com/legit-ninja/intersoccer-product-variations/pull/43) for details.

## [2.9.3] - 2026-09-22

### Fixed
- Hide internal order-item meta from customer surfaces (thank-you, emails, My Account): assigned player keys, taxonomy `attribute_pa_*` / `pa_*`, player-index leftovers, and underscore-prefixed `_camp_*` / `_intersoccer_*` keys. Admin order views and Reports & Rosters raw reads stay unchanged.
- Dual-write `_assigned_player` / `_assigned_player_id` alongside the bare keys so readers can migrate without a hard cutover.

### Added
- Program Manager year-roll UI after Duplicate to year: muted next-step helper, hold-on-complete progress overlay with Draft links, draft-only lifecycle banner, and a Scaffold CTA when variations are missing.
